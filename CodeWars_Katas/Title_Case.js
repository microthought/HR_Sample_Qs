...
be lowercase except for the first word in the string. The JavaScript/CoffeeScript tests will pass undefined when this argument is unused.
Example
...
function titleCase(title, minorWords) {
  ...
  function caseCorrector (value, index){
    if( index === 0 || minWords.indexOf(value) === -1) {
      return value.replace(value.slice(0,1), value.slice(0,1).toUpperCase());
    } else {
      return value.replace(value.slice(0,1), value.slice(0,1).toLowerCase());
    }
  }
  return titleWords.map(caseCorrector).join(" ");
}
...
var bittyWords = ("a the of if an")

var title1 = "the lord of the rings: the fellowship of the ring";
var title2 = "the hobbit: there and back again";



var aWord = "dude";
aWord = aWord.replace(aWord.slice(0,1), aWord.slice(0,1).toUpperCase());


// littler stepss
var aWord = "dude";
var letter = aWord.slice(1,0)
