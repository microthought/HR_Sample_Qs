...
directions to walk (eg. ['n', 's', 'w', 'e']). You know it takes you one 
minute to traverse one city block, so create a function that will return 
true if the walk the app gives you will take you exactly ten minutes (you 
...
function isValidWalk(walk) {
  if(walk.length === 10) {
    let n = 0, s = 0, e = 0, w = 0;
    for (var i = 0, len = walk.length; i < len; i++){
      if(walk[i].toLowerCase() === "n") n++;
      if(walk[i].toLowerCase() === "s") s++;
      if(walk[i].toLowerCase() === "e") e++;
      if(walk[i].toLowerCase() === "w") w++;
    }
